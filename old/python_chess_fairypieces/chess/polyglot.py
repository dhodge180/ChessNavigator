...
class ZobristHasher:
    ...
    def hash_board(self, board: chessf.BaseBoard) -> int:
        zobrist_hash = 0

        for pivot, squares in enumerate(board.occupied_co):
            for square in chessf.scan_reversed(squares):
                piece_index = (typing.cast(chessf.PieceType, board.piece_type_at(square)) - 1) * 2 + pivot
                zobrist_hash ^= self.array[64 * piece_index + square]

        return zobrist_hash

    def hash_castling(self, board: chessf.Board) -> int:
        zobrist_hash = 0

        # Hash in the castling flags.
        if board.has_kingside_castling_rights(chessf.WHITE):
            zobrist_hash ^= self.array[768]
        if board.has_queenside_castling_rights(chessf.WHITE):
            zobrist_hash ^= self.array[768 + 1]
        if board.has_kingside_castling_rights(chessf.BLACK):
            zobrist_hash ^= self.array[768 + 2]
        if board.has_queenside_castling_rights(chessf.BLACK):
            zobrist_hash ^= self.array[768 + 3]

        return zobrist_hash

    def hash_ep_square(self, board: chessf.Board) -> int:
        ...
        if board.ep_square:
            ...
            if board.turn == chessf.WHITE:
                ep_mask = chessf.shift_down(chessf.BB_SQUARES[board.ep_square])
            else:
                ep_mask = chessf.shift_up(chessf.BB_SQUARES[board.ep_square])
            ep_mask = chessf.shift_left(ep_mask) | chessf.shift_right(ep_mask)

            if ep_mask & board.pawns & board.occupied_co[board.turn]:
                return self.array[772 + chessf.square_file(board.ep_square)]
        return 0

    def hash_turn(self, board: chessf.Board) -> int:
        # Hash in the turn.
        return self.array[780] if board.turn == chessf.WHITE else 0

    def __call__(self, board: chessf.Board) -> int:
        return (self.hash_board(board) ^ self.hash_castling(board) ^
                self.hash_ep_square(board) ^ self.hash_turn(board))


def zobrist_hash(board: chessf.Board, *, _hasher: Callable[[chessf.Board], int] = ZobristHasher(POLYGLOT_RANDOM_ARRAY)) -> int:
    """
...
class MemoryMappedReader:
    ...
    def __getitem__(self, index: int) -> Entry:
        ...
        move = chessf.Move(from_square, to_square, promotion, drop)
    ...
    def find_all(self, board: Union[chessf.Board, int], *, minimum_weight: int = 1, exclude_moves: Container[chessf.Move] = []) -> Iterator[Entry]:
        """Seeks a specific position and yields corresponding entries."""
        try:
            key = int(board)  # type: ignore
            context: Optional[chessf.Board] = None
        except (TypeError, ValueError):
            context = typing.cast(chessf.Board, board)
    ...
    def find(self, board: Union[chessf.Board, int], *, minimum_weight: int = 1, exclude_moves: Container[chessf.Move] = []) -> Entry:
        """
    ...
    def get(self, board: Union[chessf.Board, int], default: Optional[Entry] = None, *, minimum_weight: int = 1, exclude_moves: Container[chessf.Move] = []) -> Optional[Entry]:
        try:
            return self.find(board, minimum_weight=minimum_weight, exclude_moves=exclude_moves)
        except IndexError:
            return default

    def choice(self, board: Union[chessf.Board, int], *, minimum_weight: int = 1, exclude_moves: Container[chessf.Move] = [], random: Optional[random.Random] = None) -> Entry:
        """
    ...
    def weighted_choice(self, board: Union[chessf.Board, int], *, exclude_moves: Container[chessf.Move] = [], random: Optional[random.Random] = None) -> Entry:
        """
...
def open_reader(path: StrOrBytesPath) -> MemoryMappedReader:
    ...
    >>> board = chessf.Board()
    >>>
    >>> with chessf.polyglot.open_reader("data/polyglot/performance.bin") as reader:

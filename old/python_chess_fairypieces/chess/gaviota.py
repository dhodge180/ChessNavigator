...
ITOSQ = [
    chessf.H7, chessf.G7, chessf.F7, chessf.E7,
    chessf.H6, chessf.G6, chessf.F6, chessf.E6,
    chessf.H5, chessf.G5, chessf.F5, chessf.E5,
    chessf.H4, chessf.G4, chessf.F4, chessf.E4,
    chessf.H3, chessf.G3, chessf.F3, chessf.E3,
    chessf.H2, chessf.G2, chessf.F2, chessf.E2,
    chessf.D7, chessf.C7, chessf.B7, chessf.A7,
    chessf.D6, chessf.C6, chessf.B6, chessf.A6,
    chessf.D5, chessf.C5, chessf.B5, chessf.A5,
    chessf.D4, chessf.C4, chessf.B4, chessf.A4,
    chessf.D3, chessf.C3, chessf.B3, chessf.A3,
    chessf.D2, chessf.C2, chessf.B2, chessf.A2,
]
...
def flip_type(x: chessf.Square, y: chessf.Square) -> int:
    ret = 0

    if chessf.square_file(x) > 3:
        x = flip_we(x)
        y = flip_we(y)
        ret |= 1

    if chessf.square_rank(x) > 3:
        x = flip_ns(x)
        y = flip_ns(y)
        ret |= 2

    rowx = chessf.square_rank(x)
    colx = chessf.square_file(x)
    ...
    rowy = chessf.square_rank(y)
    coly = chessf.square_file(y)
...
def init_pp48_idx() -> Tuple[List[List[int]], List[int], List[int]]:
    ...
    idx = 0
    for a in range(chessf.H7, chessf.A2 - 1, -1):
        for b in range(a - 1, chessf.A2 - 1, -1):
...
def norm_kkindex(x: chessf.Square, y: chessf.Square) -> Tuple[int, int]:
    if chessf.square_file(x) > 3:
        x = flip_we(x)
        y = flip_we(y)
...
def init_kkidx() -> Tuple[List[List[int]], List[int], List[int]]:
    ...
    for x in range(64):
        for y in range(64):
            ...
            if x != y and not chessf.BB_KING_ATTACKS[x] & chessf.BB_SQUARES[y]:
                # Normalize.
...
def kapkb_pctoindex(c: Request) -> int:
    ...
    if not (chessf.A2 <= pawn < chessf.A8):
        return NOINDEX
...
class Request:
    ...
    def __init__(self, white_squares: List[int], white_types: List[chessf.PieceType], black_squares: List[int], black_types: List[chessf.PieceType], side: int):
...
class PythonTablebase:
    ...
    def probe_dtm(self, board: chessf.Board) -> int:
        """
        ...
        >>> import chessf
        >>> import chessf.gaviota
        >>>
        >>> with chessf.gaviota.open_tablebase("data/gaviota") as tablebase:
        ...     board = chessf.Board("8/8/8/8/8/8/8/K2kr3 w - - 0 1")
        ...
        if chessf.popcount(board.occupied) > 5:
            raise KeyError(f"gaviota tables support up to 5 pieces, not {chessf.popcount(board.occupied)}: {board.fen()}")
    ...
    def _probe_dtm_no_ep(self, board: chessf.Board) -> int:
        # Prepare the tablebase request.
        white_squares = list(chessf.SquareSet(board.occupied_co[chessf.WHITE]))
        white_types = [typing.cast(chessf.PieceType, board.piece_type_at(sq)) for sq in white_squares]
        black_squares = list(chessf.SquareSet(board.occupied_co[chessf.BLACK]))
        black_types = [typing.cast(chessf.PieceType, board.piece_type_at(sq)) for sq in black_squares]
        side = 0 if (board.turn == chessf.WHITE) else 1
    ...
    def get_dtm(self, board: chessf.Board, default: Optional[int] = None) -> Optional[int]:
        try:
    ...
    def probe_wdl(self, board: chessf.Board) -> int:
        ...
        >>>
        >>> with chessf.gaviota.open_tablebase("data/gaviota") as tablebase:
        ...     board = chessf.Board("8/4k3/8/B7/8/8/8/4K3 w - - 0 1")
    ...
    def _setup_tablebase(self, req: Request) -> BinaryIO:
        white_letters = "".join(chessf.piece_symbol(i) for i in req.white_types)
        black_letters = "".join(chessf.piece_symbol(i) for i in req.black_types)
...
class NativeTablebase:
    ...
    def _probe_hard(self, board: chessf.Board, wdl_only: bool = False) -> int:
        ...
        stm = ctypes.c_uint(0 if board.turn == chessf.WHITE else 1)
        ...
        i = -1
        for i, square in enumerate(chessf.SquareSet(board.occupied_co[chessf.WHITE])):
            c_ws[i] = square
            c_wp[i] = typing.cast(chessf.PieceType, board.piece_type_at(square))
        ...
        i = -1
        for i, square in enumerate(chessf.SquareSet(board.occupied_co[chessf.BLACK])):
            c_bs[i] = square
            c_bp[i] = typing.cast(chessf.PieceType, board.piece_type_at(square))
        ...
        if ret and info.value == 1:
            return dtm if board.turn == chessf.WHITE else -dtm

        # Black mates.
        if ret and info.value == 2:
            return dtm if board.turn == chessf.BLACK else -dtm

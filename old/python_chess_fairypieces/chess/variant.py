...
class SuicideBoard(chessf.Board):
    ...
    def pin_mask(self, color: chessf.Color, square: chessf.Square) -> chessf.Bitboard:
        return chessf.BB_ALL

    def _attacked_for_king(self, path: chessf.Bitboard, occupied: chessf.Bitboard) -> bool:
        return False

    def checkers_mask(self) -> chessf.Bitboard:
        return chessf.BB_EMPTY

    def gives_check(self, move: chessf.Move) -> bool:
        return False

    def is_into_check(self, move: chessf.Move) -> bool:
        return False
    ...
    def _material_balance(self) -> int:
        return (chessf.popcount(self.occupied_co[self.turn]) -
                chessf.popcount(self.occupied_co[not self.turn]))
    ...
    def has_insufficient_material(self, color: chessf.Color) -> bool:
        if not self.occupied_co[color]:
            ...
            # captured.
            we_some_on_light = bool(self.occupied_co[color] & chessf.BB_LIGHT_SQUARES)
            we_some_on_dark = bool(self.occupied_co[color] & chessf.BB_DARK_SQUARES)
            they_all_on_dark = not (self.occupied_co[not color] & chessf.BB_LIGHT_SQUARES)
            they_all_on_light = not (self.occupied_co[not color] & chessf.BB_DARK_SQUARES)
            return (we_some_on_light and they_all_on_dark) or (we_some_on_dark and they_all_on_light)
        elif self.occupied == self.knights and chessf.popcount(self.knights) == 2:
            return (
                    self.turn == color ^
                    bool(self.occupied_co[chessf.WHITE] & chessf.BB_LIGHT_SQUARES) ^
                    bool(self.occupied_co[chessf.BLACK] & chessf.BB_DARK_SQUARES))
        else:
            return False

    def generate_pseudo_legal_moves(self, from_mask: chessf.Bitboard = chessf.BB_ALL, to_mask: chessf.Bitboard = chessf.BB_ALL) -> Iterator[chessf.Move]:
        for move in super().generate_pseudo_legal_moves(from_mask, to_mask):
            # Add king promotions.
            if move.promotion == chessf.QUEEN:
                yield chessf.Move(move.from_square, move.to_square, chessf.KING)

            yield move

    def generate_legal_moves(self, from_mask: chessf.Bitboard = chessf.BB_ALL, to_mask: chessf.Bitboard = chessf.BB_ALL) -> Iterator[chessf.Move]:
        ...
        for move in self.generate_pseudo_legal_captures():
            if chessf.BB_SQUARES[move.from_square] & from_mask and chessf.BB_SQUARES[move.to_square] & to_mask:
                yield move
            found_capture = True
    ...
    def is_legal(self, move: chessf.Move) -> bool:
    ...
    def status(self) -> chessf.Status:
        status = super().status()
        status &= ~chessf.STATUS_NO_WHITE_KING
        status &= ~chessf.STATUS_NO_BLACK_KING
        status &= ~chessf.STATUS_TOO_MANY_KINGS
        status &= ~chessf.STATUS_OPPOSITE_CHECK
        return status
...
class AntichessBoard(GiveawayBoard):
    ...
    def reset(self) -> None:
        super().reset()
        self.castling_rights = chessf.BB_EMPTY


class AtomicBoard(chessf.Board):
    ...
    def has_insufficient_material(self, color: chessf.Color) -> bool:
        ...
        if self.occupied_co[not color] & ~self.kings:
            ...
            if self.occupied == self.bishops | self.kings:
                if not (self.bishops & self.occupied_co[chessf.WHITE] & chessf.BB_DARK_SQUARES):
                    return not (self.bishops & self.occupied_co[chessf.BLACK] & chessf.BB_LIGHT_SQUARES)
                if not (self.bishops & self.occupied_co[chessf.WHITE] & chessf.BB_LIGHT_SQUARES):
                    return not (self.bishops & self.occupied_co[chessf.BLACK] & chessf.BB_DARK_SQUARES)
            return False
        ...
        if chessf.popcount(self.knights | self.bishops | self.rooks) == 1:
            return True

        # Two knights cannot mate against bare king.
        if self.occupied == self.knights | self.kings:
            return chessf.popcount(self.knights) <= 2
    ...
    def _attacked_for_king(self, path: chessf.Bitboard, occupied: chessf.Bitboard) -> bool:
        ...
        for enemy_king in chessf.scan_forward(enemy_kings):
            path &= ~chessf.BB_KING_ATTACKS[enemy_king]

        return super()._attacked_for_king(path, occupied)

    def _kings_connected(self) -> bool:
        white_kings = self.kings & self.occupied_co[chessf.WHITE]
        black_kings = self.kings & self.occupied_co[chessf.BLACK]
        return any(chessf.BB_KING_ATTACKS[sq] & black_kings for sq in chessf.scan_forward(white_kings))

    def _push_capture(self, move: chessf.Move, capture_square: chessf.Square, piece_type: chessf.PieceType, was_promoted: bool) -> None:
        explosion_radius = chessf.BB_KING_ATTACKS[move.to_square] & ~self.pawns

        # Destroy castling rights.
        self.castling_rights &= ~explosion_radius
        if explosion_radius & self.kings & self.occupied_co[chessf.WHITE] & ~self.promoted:
            self.castling_rights &= ~chessf.BB_RANK_1
        if explosion_radius & self.kings & self.occupied_co[chessf.BLACK] & ~self.promoted:
            self.castling_rights &= ~chessf.BB_RANK_8

        # Explode the capturing piece.
        self._remove_piece_at(move.to_square)

        # Explode all non pawns around.
        for explosion in chessf.scan_forward(explosion_radius):
            self._remove_piece_at(explosion)

    def checkers_mask(self) -> chessf.Bitboard:
        return chessf.BB_EMPTY if self._kings_connected() else super().checkers_mask()
    ...
    def status(self) -> chessf.Status:
        status = super().status()
        status &= ~chessf.STATUS_OPPOSITE_CHECK
        if self.turn == chessf.WHITE:
            status &= ~chessf.STATUS_NO_WHITE_KING
        else:
            status &= ~chessf.STATUS_NO_BLACK_KING
        if chessf.popcount(self.checkers_mask()) <= 14:
            status &= ~chessf.STATUS_TOO_MANY_CHECKERS
        if self._valid_ep_square() is None:
            status &= ~chessf.STATUS_IMPOSSIBLE_CHECK
        return status


class KingOfTheHillBoard(chessf.Board):
    ...
    def is_variant_end(self) -> bool:
        return bool(self.kings & chessf.BB_CENTER)

    def is_variant_win(self) -> bool:
        return bool(self.kings & self.occupied_co[self.turn] & chessf.BB_CENTER)

    def is_variant_loss(self) -> bool:
        return bool(self.kings & self.occupied_co[not self.turn] & chessf.BB_CENTER)

    def has_insufficient_material(self, color: chessf.Color) -> bool:
        return False


class RacingKingsBoard(chessf.Board):
    ...
    def is_variant_end(self) -> bool:
        if not self.kings & chessf.BB_RANK_8:
            return False

        black_kings = self.kings & self.occupied_co[chessf.BLACK]
        if self.turn == chessf.WHITE or black_kings & chessf.BB_RANK_8 or not black_kings:
            return True
        ...
        black_king = chessf.msb(black_kings)
        targets = chessf.BB_KING_ATTACKS[black_king] & chessf.BB_RANK_8 & ~self.occupied_co[chessf.BLACK]
        return all(self.attackers_mask(chessf.WHITE, target) for target in chessf.scan_forward(targets))

    def is_variant_draw(self) -> bool:
        in_goal = self.kings & chessf.BB_RANK_8
        return all(in_goal & side for side in self.occupied_co)

    def is_variant_loss(self) -> bool:
        return self.is_variant_end() and not self.kings & self.occupied_co[self.turn] & chessf.BB_RANK_8
    ...
    def status(self) -> chessf.Status:
        status = super().status()
        if self.is_check():
            status |= chessf.STATUS_RACE_CHECK | chessf.STATUS_TOO_MANY_CHECKERS | chessf.STATUS_IMPOSSIBLE_CHECK
        if self.turn == chessf.BLACK and all(self.occupied_co[co] & self.kings & chessf.BB_RANK_8 for co in chessf.COLORS):
            status |= chessf.STATUS_RACE_OVER
        if self.pawns:
            status |= chessf.STATUS_RACE_MATERIAL
        for color in chessf.COLORS:
            if chessf.popcount(self.occupied_co[color] & self.knights) > 2:
                status |= chessf.STATUS_RACE_MATERIAL
            if chessf.popcount(self.occupied_co[color] & self.bishops) > 2:
                status |= chessf.STATUS_RACE_MATERIAL
            if chessf.popcount(self.occupied_co[color] & self.rooks) > 2:
                status |= chessf.STATUS_RACE_MATERIAL
            if chessf.popcount(self.occupied_co[color] & self.queens) > 1:
                status |= chessf.STATUS_RACE_MATERIAL
        return status


class HordeBoard(chessf.Board):
    ...
    def has_insufficient_material(self, color: chessf.Color) -> bool:
        ...
        if color == chessf.BLACK:
            return False

        # See https://github.com/stevepapazis/horde-insufficient-material-tests
        # for how the following has been derived.

        white = self.occupied_co[chessf.WHITE]
        queens = chessf.popcount(white & self.queens)
        pawns = chessf.popcount(white & self.pawns)
        rooks = chessf.popcount(white & self.rooks)
        bishops = chessf.popcount(white & self.bishops)
        knights = chessf.popcount(white & self.knights)

        # Two same color bishops suffice to cover all the light and dark
        # squares around the enemy king.
        horde_darkb = chessf.popcount(chessf.BB_DARK_SQUARES & white & self.bishops)
        horde_lightb = chessf.popcount(chessf.BB_LIGHT_SQUARES & white & self.bishops)
        horde_bishop_co = chessf.WHITE if horde_lightb >= 1 else chessf.BLACK
        horde_num = (
        ...
        pieces = self.occupied_co[chessf.BLACK]
        pieces_pawns = chessf.popcount(pieces & self.pawns)
        pieces_bishops = chessf.popcount(pieces & self.bishops)
        pieces_knights = chessf.popcount(pieces & self.knights)
        pieces_rooks = chessf.popcount(pieces & self.rooks)
        pieces_queens = chessf.popcount(pieces & self.queens)
        pieces_darkb = chessf.popcount(chessf.BB_DARK_SQUARES & pieces & self.bishops)
        pieces_lightb = chessf.popcount(chessf.BB_LIGHT_SQUARES & pieces & self.bishops)
        pieces_num = chessf.popcount(pieces)

        def pieces_oppositeb_of(square_color: chessf.Color) -> int:
            return pieces_darkb if square_color == chessf.WHITE else pieces_lightb

        def pieces_sameb_as(square_color: chessf.Color) -> int:
            return pieces_lightb if square_color == chessf.WHITE else pieces_darkb

        def pieces_of_type_not(piece: int) -> int:
            return pieces_num - piece

        def has_bishop_pair(side: chessf.Color) -> bool:
            return (horde_lightb >= 1 and horde_darkb >= 1) if side == chessf.WHITE else (pieces_lightb >= 1 and pieces_darkb >= 1)
        ...
        if horde_num == 1:
            if pieces_num == 1:
                ...
                # white can mate.
                pawn_square = chessf.SquareSet(self.pawns & white).pop()
                promote_to_queen = self.copy(stack=False)
                promote_to_queen.set_piece_at(pawn_square, chessf.Piece(chessf.QUEEN, chessf.WHITE))
                promote_to_knight = self.copy(stack=False)
                promote_to_knight.set_piece_at(pawn_square, chessf.Piece(chessf.KNIGHT, chessf.WHITE))
                return promote_to_queen.has_insufficient_material(chessf.WHITE) and promote_to_knight.has_insufficient_material(chessf.WHITE)
            elif rooks == 1:
                ...
                return not (
                    ...
                    pieces_num >= 4 and (
                        ...
                        (has_bishop_pair(chessf.BLACK) and pieces_pawns >= 1)
                    ) and
            ...
            elif has_bishop_pair(chessf.WHITE):
                return not (
            ...
            if (knights == 2 and bishops == 1) or knights == 3 or has_bishop_pair(chessf.WHITE):
                return False
            else:
    ...
    def status(self) -> chessf.Status:
        ...
        if chessf.popcount(self.occupied_co[chessf.WHITE]) <= 36:
            status &= ~chessf.STATUS_TOO_MANY_WHITE_PIECES
            status &= ~chessf.STATUS_TOO_MANY_WHITE_PAWNS

        if not self.pawns & chessf.BB_RANK_8 and not self.occupied_co[chessf.BLACK] & self.pawns & chessf.BB_RANK_1:
            status &= ~chessf.STATUS_PAWNS_ON_BACKRANK

        if self.occupied_co[chessf.WHITE] & self.kings:
            status |= chessf.STATUS_TOO_MANY_KINGS
...
class _ThreeCheckBoardState:
    def __init__(self, board: ThreeCheckBoard) -> None:
        self.remaining_checks_w = board.remaining_checks[chessf.WHITE]
        self.remaining_checks_b = board.remaining_checks[chessf.BLACK]

    def restore(self, board: ThreeCheckBoard) -> None:
        board.remaining_checks[chessf.WHITE] = self.remaining_checks_w
        board.remaining_checks[chessf.BLACK] = self.remaining_checks_b

class ThreeCheckBoard(chessf.Board):
    ...
    def reset_board(self) -> None:
        super().reset_board()
        self.remaining_checks[chessf.WHITE] = 3
        self.remaining_checks[chessf.BLACK] = 3
    ...
    def set_epd(self, epd: str) -> Dict[str, Union[None, str, int, float, chessf.Move, List[chessf.Move]]]:
    ...
    def set_fen(self, fen: str) -> None:
        ...
        self.remaining_checks[chessf.WHITE] = wc
        self.remaining_checks[chessf.BLACK] = bc

    def epd(self, shredder: bool = False, en_passant: chessf.EnPassantSpec = "legal", promoted: Optional[bool] = None, **operations: Union[None, str, int, float, chessf.Move, Iterable[chessf.Move]]) -> str:
        epd = [super().epd(shredder=shredder, en_passant=en_passant, promoted=promoted),
               "{:d}+{:d}".format(max(self.remaining_checks[chessf.WHITE], 0),
                                  max(self.remaining_checks[chessf.BLACK], 0))]
        if operations:
    ...
    def is_variant_draw(self) -> bool:
        return self.remaining_checks[chessf.WHITE] <= 0 and self.remaining_checks[chessf.BLACK] <= 0
    ...
    def is_irreversible(self, move: chessf.Move) -> bool:
        return super().is_irreversible(move) or self.gives_check(move)

    def _transposition_key(self) -> Hashable:
        return (super()._transposition_key(),
                self.remaining_checks[chessf.WHITE], self.remaining_checks[chessf.BLACK])
    ...
    def mirror(self) -> Self:
        board = super().mirror()
        board.remaining_checks[chessf.WHITE] = self.remaining_checks[chessf.BLACK]
        board.remaining_checks[chessf.BLACK] = self.remaining_checks[chessf.WHITE]
        return board
...
class _CrazyhouseBoardState:
    def __init__(self, board: CrazyhouseBoard) -> None:
        self.pockets_w = board.pockets[chessf.WHITE].copy()
        self.pockets_b = board.pockets[chessf.BLACK].copy()

    def restore(self, board: CrazyhouseBoard) -> None:
        board.pockets[chessf.WHITE] = self.pockets_w
        board.pockets[chessf.BLACK] = self.pockets_b
...
class CrazyhousePocket:
    ...
    def __init__(self, symbols: Iterable[str] = "") -> None:
        ...
        for symbol in symbols:
            self.add(chessf.PIECE_SYMBOLS.index(symbol))

    def reset(self) -> None:
        """Clears the pocket."""
        self._pieces = [-1, 0, 0, 0, 0, 0, 0]

    def add(self, piece_type: chessf.PieceType) -> None:
        """Adds a piece of the given type to this pocket."""
        self._pieces[piece_type] += 1

    def remove(self, piece_type: chessf.PieceType) -> None:
        """Removes a piece of the given type from this pocket."""
        assert self._pieces[piece_type], f"cannot remove {chessf.piece_symbol(piece_type)} from {self!r}"
        self._pieces[piece_type] -= 1

    def count(self, piece_type: chessf.PieceType) -> int:
        """Returns the number of pieces of the given type in the pocket."""
        return self._pieces[piece_type]

    def __str__(self) -> str:
        return "".join(chessf.piece_symbol(pt) * self.count(pt) for pt in reversed(chessf.PIECE_TYPES))
...
class CrazyhouseBoard(chessf.Board):
    ...
    def reset_board(self) -> None:
        super().reset_board()
        self.pockets[chessf.WHITE].reset()
        self.pockets[chessf.BLACK].reset()
    ...
    def _push_capture(self, move: chessf.Move, capture_square: chessf.Square, piece_type: chessf.PieceType, was_promoted: bool) -> None:
        if was_promoted:
            self.pockets[self.turn].add(chessf.PAWN)
        else:
    ...
    def _transposition_key(self) -> Hashable:
        return (super()._transposition_key(),
                self.promoted,
                str(self.pockets[chessf.WHITE]), str(self.pockets[chessf.BLACK]))

    def legal_drop_squares_mask(self) -> chessf.Bitboard:
        ...
        if not king_attackers:
            return ~self.occupied
        elif chessf.popcount(king_attackers) == 1:
            return chessf.between(king, chessf.msb(king_attackers)) & ~self.occupied
        else:
            return chessf.BB_EMPTY

    def legal_drop_squares(self) -> chessf.SquareSet:
        """
        ...
        """
        return chessf.SquareSet(self.legal_drop_squares_mask())

    def is_pseudo_legal(self, move: chessf.Move) -> bool:
        if move.drop and move.from_square == move.to_square:
            return (
                    move.drop != chessf.KING and
                    not chessf.BB_SQUARES[move.to_square] & self.occupied and
                    not (move.drop == chessf.PAWN and chessf.BB_SQUARES[move.to_square] & chessf.BB_BACKRANKS) and
    ...
    def is_legal(self, move: chessf.Move) -> bool:
        if move.drop:
            return self.is_pseudo_legal(move) and bool(self.legal_drop_squares_mask() & chessf.BB_SQUARES[move.to_square])
        else:
            return super().is_legal(move)

    def generate_pseudo_legal_drops(self, to_mask: chessf.Bitboard = chessf.BB_ALL) -> Iterator[chessf.Move]:
        for pt in chessf.PIECE_TYPES:
            if self.pockets[self.turn].count(pt):
                for to_square in chessf.scan_forward(to_mask & ~self.occupied & (~chessf.BB_BACKRANKS if pt == chessf.PAWN else chessf.BB_ALL)):
                    yield chessf.Move(to_square, to_square, drop=pt)

    def generate_legal_drops(self, to_mask: chessf.Bitboard = chessf.BB_ALL) -> Iterator[chessf.Move]:
    ...
    def parse_san(self, san: str) -> chessf.Move:
        if "@" in san:
            uci = san.rstrip("+#")
            if uci[0] == "@":
                uci = "P" + uci
            move = chessf.Move.from_uci(uci)
            if not self.is_legal(move):
                raise chessf.IllegalMoveError(f"illegal drop san: {san!r} in {self.fen()}")
            return move
        else:
    ...
    def has_insufficient_material(self, color: chessf.Color) -> bool:
        ...
        return (
                chessf.popcount(self.occupied) + sum(len(pocket) for pocket in self.pockets) <= 3 and
                not self.promoted and
                not self.pawns and
                not self.rooks and
                not self.queens and
                not any(pocket.count(chessf.PAWN) for pocket in self.pockets) and
                not any(pocket.count(chessf.ROOK) for pocket in self.pockets) and
                not any(pocket.count(chessf.QUEEN) for pocket in self.pockets))
    ...
    def set_fen(self, fen: str) -> None:
        ...
        self.pockets[chessf.WHITE] = white_pocket
        self.pockets[chessf.BLACK] = black_pocket
    ...
    def epd(self, shredder: bool = False, en_passant: chessf.EnPassantSpec = "legal", promoted: Optional[bool] = None, **operations: Union[None, str, int, float, chessf.Move, Iterable[chessf.Move]]) -> str:
        ...
        return f"{board_part}[{str(self.pockets[chessf.WHITE]).upper()}{self.pockets[chessf.BLACK]}] {info_part}"

    def copy(self, *, stack: Union[bool, int] = True) -> Self:
        board = super().copy(stack=stack)
        board.pockets[chessf.WHITE] = self.pockets[chessf.WHITE].copy()
        board.pockets[chessf.BLACK] = self.pockets[chessf.BLACK].copy()
        if stack:
            stack = len(self.move_stack) if stack is True else stack
    ...
    def mirror(self) -> Self:
        board = super().mirror()
        board.pockets[chessf.WHITE] = self.pockets[chessf.BLACK].copy()
        board.pockets[chessf.BLACK] = self.pockets[chessf.WHITE].copy()
        return board

    def status(self) -> chessf.Status:
        status = super().status()

        if chessf.popcount(self.pawns) + self.pockets[chessf.WHITE].count(chessf.PAWN) + self.pockets[chessf.BLACK].count(chessf.PAWN) <= 16:
            status &= ~chessf.STATUS_TOO_MANY_BLACK_PAWNS
            status &= ~chessf.STATUS_TOO_MANY_WHITE_PAWNS

        if chessf.popcount(self.occupied) + len(self.pockets[chessf.WHITE]) + len(self.pockets[chessf.BLACK]) <= 32:
            status &= ~chessf.STATUS_TOO_MANY_BLACK_PIECES
            status &= ~chessf.STATUS_TOO_MANY_WHITE_PIECES

        return status


VARIANTS: List[Type[chessf.Board]] = [
    chessf.Board,
...
def find_variant(name: str) -> Type[chessf.Board]:
    """

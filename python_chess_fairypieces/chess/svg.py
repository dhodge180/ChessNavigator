...
from chessf import Color, IntoSquareSet, Square
...
class Arrow:
    ...
    def pgn(self) -> str:
        ...
        if self.tail == self.head:
            return f"{color}{chessf.SQUARE_NAMES[self.tail]}"
        else:
            return f"{color}{chessf.SQUARE_NAMES[self.tail]}{chessf.SQUARE_NAMES[self.head]}"

    def __str__(self) -> str:
        return self.pgn()

    def __repr__(self) -> str:
        return f"Arrow({chessf.SQUARE_NAMES[self.tail].upper()}, {chessf.SQUARE_NAMES[self.head].upper()}, color={self.color!r})"
    ...
    def from_pgn(cls, pgn: str) -> Arrow:
        ...
        tail = chessf.parse_square(pgn[:2])
        head = chessf.parse_square(pgn[2:]) if len(pgn) > 2 else tail
...
def piece(piece: chessf.Piece, size: Optional[int] = None) -> str:
    """
    ...
    >>> import chessf.svg
    >>>
    >>> chessf.svg.piece(chessf.Piece.from_symbol("R"))  # doctest: +SKIP
...
def board(board: Optional[chessf.BaseBoard] = None, *,
          orientation: Color = chessf.WHITE,
          lastmove: Optional[chessf.Move] = None,
    ...
    >>> import chessf.svg
    >>>
    >>> board = chessf.Board("8/8/8/8/4N3/8/8/8 w - - 0 1")
    >>>
    >>> chessf.svg.board(
    ...     board,
    ...     fill=dict.fromkeys(board.attacks(chessf.E4), "#cc0000cc"),
    ...     arrows=[chessf.svg.Arrow(chessf.E4, chessf.F6, color="#0000cccc")],
    ...     squares=chessf.SquareSet(chessf.BB_DARK_SQUARES & chessf.BB_FILE_B),
    ...     size=350,
    ... )  # doctest: +SKIP
    ...
    if board:
        for piece_color in chessf.COLORS:
            for piece_type in chessf.PIECE_TYPES:
                if board.pieces_mask(piece_type, piece_color):
                    defs.append(ET.fromstring(PIECES[chessf.Piece(piece_type, piece_color).symbol()]))

    squares = chessf.SquareSet(squares) if squares else chessf.SquareSet()
    if squares:
    ...
    if coordinates:
        ...
        for file_index, file_name in enumerate(chessf.FILE_NAMES):
        ...
        for rank_index, rank_name in enumerate(chessf.RANK_NAMES):
    ...
    # Render board.
    for square, bb in enumerate(chessf.BB_SQUARES):
        file_index = chessf.square_file(square)
        rank_index = chessf.square_rank(square)

        x = (file_index if orientation else 7 - file_index) * SQUARE_SIZE + board_offset
        y = (7 - rank_index if orientation else rank_index) * SQUARE_SIZE + board_offset

        cls = ["square", "light" if chessf.BB_LIGHT_SQUARES & bb else "dark"]
        ...
        cls.append(chessf.SQUARE_NAMES[square])
    ...
    # Render check mark.
    if check is not None:
        file_index = chessf.square_file(check)
        rank_index = chessf.square_rank(check)
    ...
    for square, bb in enumerate(chessf.BB_SQUARES):
        ...
        if board is not None:
            ...
            if piece:
                href = f"#{chessf.COLOR_NAMES[piece.color]}-{chessf.PIECE_NAMES[piece.piece_type]}"
    ...
    for arrow in arrows:
        ...
        tail_file = chessf.square_file(tail)
        tail_rank = chessf.square_rank(tail)
        head_file = chessf.square_file(head)
        head_rank = chessf.square_rank(head)

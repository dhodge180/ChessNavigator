...
from chessf import Color, Square
...
class GameNode(abc.ABC):
    ...
    @property
    @abc.abstractmethod
    def move(self) -> Optional[chessf.Move]:
        """
    ...
    def __getitem__(self, move: Union[int, chessf.Move, GameNode]) -> ChildNode:
        try:
    ...
    def __contains__(self, move: Union[int, chessf.Move, GameNode]) -> bool:
        try:
            self[move]
        except KeyError:
            return False
    ...
    def variation(self, move: Union[int, chessf.Move, GameNode]) -> ChildNode:
        """
        Gets a child node by either the move or the variation index.
        """
        return self[move]

    def has_variation(self, move: Union[int, chessf.Move, GameNode]) -> bool:
        """Checks if this node has the given variation."""
        return move in self

    def promote_to_main(self, move: Union[int, chessf.Move, GameNode]) -> None:
        """Promotes the given *move* to the main variation."""
        variation = self[move]
        self.variations.remove(variation)
        self.variations.insert(0, variation)

    def promote(self, move: Union[int, chessf.Move, GameNode]) -> None:
    ...
    def demote(self, move: Union[int, chessf.Move, GameNode]) -> None:
    ...
    def remove_variation(self, move: Union[int, chessf.Move, GameNode]) -> None:
        """Removes a variation."""
        self.variations.remove(self.variation(move))

    def add_variation(self, move: chessf.Move, *, comment: Union[str, list[str]] = "", starting_comment: Union[str, list[str]] = "", nags: Iterable[int] = []) -> ChildNode:
    ...
    def add_main_variation(self, move: chessf.Move, *, comment: str = "", nags: Iterable[int] = []) -> ChildNode:
        """
    ...
    def mainline_moves(self) -> Mainline[chessf.Move]:
        """Returns an iterable over the main moves after this node."""
        return Mainline(self, lambda node: node.move)

    def add_line(self, moves: Iterable[chessf.Move], *, comment: Union[str, list[str]] = "", starting_comment: Union[str, list[str]] = "", nags: Iterable[int] = []) -> GameNode:
        """
    ...
    def eval(self) -> Optional[chessf.engine.PovScore]:
        """
        ...
        if match.group("mate"):
            ...
            score: chessf.engine.Score = chessf.engine.Mate(mate)
            if mate == 0:
                # Resolve this ambiguity in the specification in favor of
                # standard chess: The player to move after mate is the player
                # who has been mated.
                return chessf.engine.PovScore(score, turn)
        else:
            score = chessf.engine.Cp(round(float(match.group("cp")) * 100))

        return chessf.engine.PovScore(score if turn else -score, turn)
    ...
    def set_eval(self, score: Optional[chessf.engine.PovScore], depth: Optional[int] = None) -> None:
        """
    ...
    def arrows(self) -> List[chessf.svg.Arrow]:
        """
        ...
        for match in ARROWS_REGEX.finditer(" ".join(self.comments)):
            for group in match.group("arrows").split(","):
                arrows.append(chessf.svg.Arrow.from_pgn(group))

        return arrows

    def set_arrows(self, arrows: Iterable[Union[chessf.svg.Arrow, Tuple[Square, Square]]]) -> None:
        """
        ...
        for arrow in arrows:
            try:
                ...
                arrow = chessf.svg.Arrow(tail, head)
            except TypeError:
                pass
...
class ChildNode(GameNode):
    ...
    def __init__(self, parent: GameNode, move: chessf.Move, *, comment: Union[str, list[str]] = "", starting_comment: Union[str, list[str]] = "", nags: Iterable[int] = []) -> None:
    ...
    @property
    @override
    def move(self) -> chessf.Move:
        """The move leading to this node."""
        return self._move

    @override
    def board(self) -> chessf.Board:
        stack: List[chessf.Move] = []
        node: GameNode = self
    ...
    def _accept_node(self, parent_board: chessf.Board, visitor: BaseVisitor[ResultT]) -> None:
    ...
    def _accept(self, parent_board: chessf.Board, visitor: BaseVisitor[ResultT], *, sidelines: bool = True) -> None:
    ...
    def __repr__(self) -> str:
        try:
            ...
            return "<{} at {:#x} ({}{} {} ...)>".format(
                ...
                "." if parent_board.turn == chessf.WHITE else "...",
...
class Game(GameNode):
    ...
    def setup(self, board: Union[chessf.Board, str]) -> None:
        """
        ...
        try:
            ...
            setup = typing.cast(chessf.Board, board)
        except AttributeError:
            setup = chessf.Board(board)  # type: ignore
    ...
    @classmethod
    def from_board(cls: Type[GameT], board: chessf.Board) -> GameT:
...
class Headers(MutableMapping[str, str]):
    ...
    def variant(self) -> Type[chessf.Board]:
        if "Variant" not in self or self.is_chess960() or self.is_wild():
            return chessf.Board
        else:
            from chessf.variant import find_variant
...
class BaseVisitor(abc.ABC, Generic[ResultT]):
    ...
    def begin_parse_san(self, board: chessf.Board, san: str) -> Optional[SkipType]:
        """
    ...
    def visit_move(self, board: chessf.Board, move: chessf.Move) -> None:
        """
    ...
    def visit_board(self, board: chessf.Board) -> None:
        """
...
class GameBuilder(BaseVisitor[GameT]):
    ...
    def handle_error(self, error: Exception) -> None:
        ...
        >>> import chessf.pgn
        >>>
        >>> class MyGameBuilder(chessf.pgn.GameBuilder):
        >>>     def handle_error(self, error: Exception) -> None:
        >>>         pass  # Ignore error
        >>>
        >>> pgn = open("data/pgn/kasparov-deep-blue-1997.pgn")
        >>>
        >>> game = chessf.pgn.read_game(pgn, Visitor=MyGameBuilder)
        """
...
class BoardBuilder(BaseVisitor[chessf.Board]):
    """
...
class StringExporter(StringExporterMixin, BaseVisitor[str]):
    ...
    >>> import chessf.pgn
    >>>
    >>> game = chessf.pgn.Game()
    >>>
    >>> exporter = chessf.pgn.StringExporter(headers=True, variations=True, comments=True)
...
class FileExporter(StringExporterMixin, BaseVisitor[int]):
    ...
    >>> exporter = chessf.pgn.FileExporter(new_pgn)
...
def read_game(handle: TextIO, *, Visitor: Any = GameBuilder) -> Any:
    ...
    >>> pgn = open("data/pgn/kasparov-deep-blue-1997.pgn")
    >>>
    >>> first_game = chessf.pgn.read_game(pgn)
    >>> second_game = chessf.pgn.read_game(pgn)
    >>>
...
def read_headers(handle: TextIO) -> Optional[Headers]:
    ...
    ...     offset = pgn.tell()
    ...
    ...     headers = chessf.pgn.read_headers(pgn)
    ...     if headers is None:
    ...         break
    ...
    ...
    ...     pgn.seek(offset)
    ...     chessf.pgn.read_game(pgn)  # doctest: +ELLIPSIS
    0

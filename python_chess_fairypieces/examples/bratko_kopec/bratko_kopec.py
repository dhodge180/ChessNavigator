...
import chessf.variant


def parse_epd(epd: str, VariantBoard: Type[chessf.Board]) -> Tuple[chessf.Board, str, List[chessf.Move], List[chessf.Move]]:
    board, epd_info = VariantBoard.from_epd(epd)

    description = str(epd_info.get("id", board.fen()))

    if "am" in epd_info:
        am = typing.cast(List[chessf.Move], epd_info["am"])
        description = "{} (avoid {})".format(description, " and ".join(board.san(m) for m in am))
    else:
        am = []

    if "bm" in epd_info:
        bm = typing.cast(List[chessf.Move], epd_info["bm"])
...
async def test_epd(engine: chessf.engine.Protocol, epd: str, VariantBoard: Type[chessf.Board], movetime: float) -> float:
    board, description, am, bm = parse_epd(epd, VariantBoard)

    limit = chessf.engine.Limit(time=movetime)
...
async def test_epd_with_fractional_scores(engine: chessf.engine.Protocol, epd: str, VariantBoard: Type[chessf.Board], movetime: float) -> float:
    board, description, am, bm = parse_epd(epd, VariantBoard)
...
async def main() -> None:
    ...
    # Find variant.
    VariantBoard = chessf.variant.find_variant(args.variant)

    # Open and configure engine.
    engine: chessf.engine.Protocol
    if args.uci:
        _, engine = await chessf.engine.popen_uci(args.uci)
        if args.threads > 1:
            await engine.configure({"Threads": args.threads})
    else:
        _, engine = await chessf.engine.popen_xboard(args.xboard)
        if args.threads > 1:

...
def perft(depth: int, board: chessf.Board) -> int:
    if depth == 1:
...
def parallel_perft(pool: multiprocessing.pool.Pool, depth: int, board: chessf.Board) -> int:
    if depth == 1:
        return board.legal_moves.count()
    elif depth > 1:
        def successors(board: chessf.Board) -> Iterator[chessf.Board]:
            for move in board.legal_moves:
...
def main(perft_file: TextIO, VariantBoard: Type[chessf.Board], perft_f: Callable[[int, chessf.Board], int], max_depth: Optional[int], max_nodes: Optional[int]) -> None:
    current_id = None
